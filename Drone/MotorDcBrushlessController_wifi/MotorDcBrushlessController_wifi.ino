...
void setup() {
  ...
  */
    ...
    escs[0].attach(12);  // attaches the servo on pin 9 to the motor FrontLeft
    escs[1].attach(11);  // attaches the servo on pin 10 to the motor FrontRight
    escs[2].attach(10);  // attaches the servo on pin 11 to the motor BackRight
    escs[3].attach(9);  // attaches the servo on pin 12 to the motor BackLeft
...
void SetMortosToZero()
{
    ...
    for (short motorIndex = 0; motorIndex < 4; motorIndex++)
    {
      currentSpeeds[motorIndex] = ESC_LOW_DEFAULT;
      escs[motorIndex].write(currentSpeeds[motorIndex]);
    }
